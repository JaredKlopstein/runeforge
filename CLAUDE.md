...
## Phase D: ways to win (v0.11.0)

- **Victory modes** (`VICS`, `CFG.vic`, stored as `G.vic`, saved): `conquest` (always active), `relic` (hold the Rune Stone 360 s; `G.relic.hold[team]` accumulates, the other side resets, both decay ×2 when unowned; 60 s warning; `#r-relic` shows `held 2:31/6:00` and is always visible in this mode), `wonder` (`BLD_DEF.wonder` 4×4, Rune age, only in the build list when `G.vic==='wonder'`; `finishBuild` sets `G.wonder={team,id,t:480}`; `#r-wonder` pill; destroyed → cleared with a toast; `G.overWhy` drives the end-screen sentence). AI: builds a wonder when rich in Rune, marches on a known player wonder, contests the relic with 5 units every 60 s in relic mode.
- **Map styles** (`MAPS`, `CFG.map`, `G.map`, saved; `genWorld(seed,style)` post-processes after the road): `river` = water band on the anti-diagonal except road tiles (bridge) and two fords 16 tiles either side of centre; `plateau` = ring of tier-2/3 stone/ore at radius 11–13 around centre with gaps on both axes and on the road. `initWorld(seed,map,vic)`; lobby has Victory and Map selectors (`paintDiff` paints all three rows).
- **Boons** (`PERKS[age]`, `team.perks`, `perk(t,id)`): at Iron `lore` (+10% XP in `addXp`) or `homes` (+2 pop per house in `popCap`); at Rune `drill` (queue timers ×1.25 in `updateBld`) or `runes` (relic food 1.3/s and ×1.25 XP). `offerPerk(age)` opens `#perk` (pauses, `UI.screen='perk'`, Esc ignored); the AI picks the first option (random on Squire). Shown in the Age dropdown.
- **Hotkeys**: `KEY_DEF` defaults, `CFG.keys` overrides, `applyKeys()` fills `KEYS` and rewrites `BLD_DEF/UNIT_DEF .key` badges; the keydown handler compares against `KEYS.*`; `hk(name)` gives the badge letter for literal buttons. Settings paints one input per action (`paintKeys`), swaps on collision, Reset clears. Also in settings: `CFG.scroll` (camera speed multiplier), `CFG.cb` (`applyTeamCol` swaps `TEAM_COL[1]` to orange and rebuilds the minimap), `CFG.autosave`.
...
## Phase E: heroes, goals, replays (v0.12.0)

- **Determinism**: `rnd()` wraps a `mulberry32` stream (`RNG`, seeded from the map seed in `initWorld`; `RNG_N` counts calls and is saved so a loaded game re-seeds and fast-forwards). Every `Math.random` in the sim slice (WORLD … AI sections) became `rnd()` except four cosmetic/visibility-dependent ones (gather tick sound, movement dust, splat jitter, "lost" toast). The AI's Squire boon pick uses `rnd()`. **Never add `Math.random` to sim code; never let sim code branch on `tileVisible`, camera, `SND`, or wall-clock time.**
- **Fixed step**: `frame` accumulates `dt*speed` into `G.acc` and runs `update(STEP)` (`STEP=1/30`) up to 8 times per frame. `G.tick` counts updates.
- **Replay**: `newGame` creates `G.replay={seed,map,vic,diff,cmds}` (not when loading a save). `logCmd(kind,args)` records `[tick, kind, args, selection ids, shift, mode]` from `issueCommand` ('cmd'), `doAction` ('act', excluding build:/amove/patrol which only set UI state), `tryPlace` ('place'), `placeWallLine` ('wall'), `resDropAction` ('drop'). `endGame`/resign call `saveReplay` → `runeforge_replay_last`. `startReplay(rep)` sets `UI.replay`, starts a fresh game with the recorded seed/map/vic/diff, and `replayApply()` (called at the top of `update`) replays commands whose tick ≤ `G.tick` with `REPLAYING=true`; the five entry points return early when `UI.replay&&!REPLAYING`. Title button `#b-replay`, end-screen `#end-replay`, `#replaybar` banner, autosave off during replay. Verified: recording and playback hashes match over 5-minute scripted matches.
- **Heroes**: the second starting villager of each team gets `u.hero={name,sk}` (`HERO_NAMES`, `HERO_SKILLS`, chosen with `rnd()`), starts at level 5 in that skill, +25% XP in it (`addXp`), a gold circlet and name label in `drawUnit`, a starred card title, a death toast; saved as `hero`.
- **Goals**: `OBJ_POOL` (16 `{id,t,done(),rw}`), `G.objs={active,done,t}` created synchronously in `newGame` (`objInit`, three at a time via `objNext` using `rnd()` so replays match), `objTick` every sim-second grants `rw` resources and toasts, `objShow` paints `#objs` (collapsible, hidden in the tutorial and on narrow screens). Saved as `objs` + `objRelicT`.
